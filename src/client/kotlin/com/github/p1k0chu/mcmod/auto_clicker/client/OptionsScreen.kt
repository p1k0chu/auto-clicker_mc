...
class OptionsScreen(name: String? = null) : Screen(Component.nullToEmpty(name ?: "Auto Clicker Settings")) {
    ...
    private fun delayFieldResponder(widget: EditBox, config: Config.SharedConfig, value: String) {
        if (value.startsWith("0") && value.length > 1) {
            widget.value = value.dropWhile { ch -> ch == '0' }
            return
        }

        try {
            config.cooldown = value.toInt()
        } catch (_: Exception) {
            widget.value = "0"
        }
    ...
    protected override fun init() {
        ...
        // spamming toggle
        tooltipsMap[this.addRenderableWidget(
        ...
        val leftDelayWidget = EditBox(
            font,
            width / 2 - 160, height / 2 - 8,
            100, 20,
            Component.nullToEmpty(AutoClicker.config.leftMouse.cooldown.toString())
        )
        leftDelayWidget.value = AutoClicker.config.leftMouse.cooldown.toString()
        tooltipsMap[this.addRenderableWidget(leftDelayWidget)] = Language.COOLDOWN.text

        leftDelayWidget.setResponder { value -> delayFieldResponder(leftDelayWidget, AutoClicker.config.leftMouse, value) }

        // ignore blocks toggle
        tooltipsMap[this.addRenderableWidget(Button.Builder(Language.IGNORE_BLOCKS.getText(AutoClicker.config.leftMouse.ignoreBlocks)) { btn: Button ->
        ...
        tooltipsMap[this.addRenderableWidget(Button.Builder(Language.IGNORE_SHIELD.getText(AutoClicker.config.leftMouse.ignoreShield)) { btn: Button ->
        ...
        tooltipsMap[this.addRenderableWidget(Button.Builder(Language.RESPECT_WEAPON_COOLDOWN.getText(AutoClicker.config.leftMouse.respectWeaponCooldown)) { btn: Button ->
        ...
        val rightDelayWidget = EditBox(
            font,
            width / 2 - 50, height / 2 - 8,
            100, 20,
            Component.nullToEmpty(AutoClicker.config.rightMouse.cooldown.toString())
        )
        rightDelayWidget.value = AutoClicker.config.rightMouse.cooldown.toString()
        tooltipsMap[this.addRenderableWidget(rightDelayWidget)] = Language.COOLDOWN.text

        rightDelayWidget.setResponder { value -> delayFieldResponder(rightDelayWidget, AutoClicker.config.rightMouse, value) }

        // ignore blocks toggle
        tooltipsMap[this.addRenderableWidget(Button.Builder(Language.IGNORE_BLOCKS.getText(AutoClicker.config.rightMouse.ignoreBlocks)) { btn: Button ->
        ...
        val jumpDelayWidget = EditBox(
            font,
            width / 2 + 60, height / 2 - 8,
            100, 20,
            Component.nullToEmpty(AutoClicker.config.jump.cooldown.toString())
        )
        jumpDelayWidget.value = AutoClicker.config.jump.cooldown.toString()
        tooltipsMap[this.addRenderableWidget(jumpDelayWidget)] = Language.COOLDOWN.text
